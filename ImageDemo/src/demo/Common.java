...
public class Common extends Frame {
	...
	public int[] grabber(Image im, int iw, int ih) {
		...
		try {
		 ...
		 {
			 System.out.println(Integer.toBinaryString(pix[i]));
		 }
*/
		return pix;
	}
	...
	public void chooseFile(JFileChooser jfc, String str, int n) {
		...
		jfc.setFileFilter(new javax.swing.filechooser.FileFilter() {
			public boolean accept(File f) {
				...
				{
					return name.endsWith(".raw") || f.isDirectory();
				} else if (type == 2) // 仅PGM,RAW
				{
					return name.endsWith(".pgm") || name.endsWith(".raw")
							|| f.isDirectory();
				} else if (type == 3) // 包含RAW和内置方法
				{
					return name.endsWith(".gif") || name.endsWith(".jpg")
							|| name.endsWith(".png") || name.endsWith(".raw")
							|| f.isDirectory();
				} else // 包含BMP,PGM和内置方法
				{
					return name.endsWith(".bmp") || name.endsWith(".gif")
							|| name.endsWith(".jpg") || name.endsWith(".pgm")
							|| name.endsWith(".png") || name.endsWith(".ppm")
							|| name.endsWith(".raw") || f.isDirectory();
				}
			}
	...
	public void draw(Graphics g, Image im, char a0, char a1, int iw, int ih,
			int maxpix) {
	...
	public boolean draw(Graphics g, Image iImage, String ents, double entr,
			String avrs, double aver, String meds, double medn, String sqss,
			double sqsm) {
		g.clearRect(0, 0, 530, 350);
		g.setColor(Color.blue);
		g.setFont(new Font("", Font.BOLD, 18));
	...
	/*************************************
	 * 显示两幅图与第2幅图像标题 im -- 输入图 istr -- 输入图标题 om -- 输出图 ostr -- 输出图标题
	 *************************************/
	...
	// 显示两幅图,带有类型type
	public void draw(Graphics g, Image im, Image om, int iw, int ih, int owh,
			int type) {
		if (type == 4) {
			g.clearRect(0, 0, 530, 330);
	...
	// 显示两幅图、数据和说明
	public void drawIm(Graphics g, Image image, Image oImage, int startX,
			int startY, int sc1, int sc0) {
	 ...
	 *----------------------------------------*/
	public void draw(Graphics g, Image iImage, BufferedImage bImage, float x,
			float y, int type) {
		g.clearRect(0, 0, 530, 330);
	...
	public double getStartValue() {
		...
		String aValue = JOptionPane.showInputDialog(null, "输入初始值(0.001-0.999)",
				tmpValue);
		// 将字符串转变为数字
		if (aValue != null)
